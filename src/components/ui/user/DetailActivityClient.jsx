...
const exampleReviews = [
  { id: 1, user: { name: "Budi Santoso", avatarUrl: "https://github.com/shadcn.png" }, rating: 5, comment: "Pengalaman yang luar biasa!", createdAt: "2024-05-20T10:00:00Z" },
  { id: 2, user: { name: "Citra Lestari", avatarUrl: "https://github.com/shadcn.png" }, rating: 4, comment: "Sangat menyenangkan untuk keluarga.", createdAt: "2024-05-18T14:30:00Z" },
];
...
export function DetailActivityClient({ initialActivity }) {
  ...
  if (!detailActivity) {
    // Tampilkan pesan atau loader jika data belum siap
      ...
      <div className="container px-4 mx-auto max-w-7xl">
        <Breadcrumb className="mb-8">
            <BreadcrumbList>
                ...
                <BreadcrumbItem><BreadcrumbLink asChild><Link href="/activity">Aktivitas</Link></BreadcrumbLink></BreadcrumbItem>
                <BreadcrumbSeparator><ChevronRight /></BreadcrumbSeparator>
          ...
          <div className="lg:w-2/3">
            <div className="p-6 bg-white rounded-lg shadow-md">
              ...
              <div className="flex flex-wrap items-center gap-4 mb-4 text-gray-600">
                <div className="flex items-center gap-1"><Star className="w-5 h-5 text-yellow-500 fill-yellow-500" /><span className="font-semibold">{detailActivity.rating}</span><span className="text-sm text-gray-500">({detailActivity.total_reviews} ulasan)</span></div>
            ...
            <div className="mt-6 overflow-hidden bg-white rounded-lg shadow-md"><div className="relative w-full h-80 md:h-96"><img src={detailActivity.imageUrls?.[0] || DEFAULT_ACTIVITY_IMAGE} onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = "/assets/error.png"; }} alt={detailActivity.title || "Gambar aktivitas"} className="object-cover w-full h-full" /></div></div>
            <div className="p-6 mt-6 bg-white rounded-lg shadow-md"><h2 className="mb-4 text-2xl font-bold text-gray-800">Tentang Aktivitas Ini</h2><p className="text-justify text-gray-700 whitespace-pre-line">{detailActivity.description}</p></div>
            <div className="p-6 mt-6 bg-white rounded-lg shadow-md"><h2 className="mb-4 text-2xl font-bold text-gray-800">Lokasi</h2><div className="flex items-center gap-2 mb-4"><MapPin className="w-5 h-5 text-gray-600" /><p className="text-gray-700">{detailActivity.address}</p></div><div className="flex justify-center w-full h-full text-gray-500 bg-white rounded-md"><div className="w-full overflow-hidden rounded-md [&>iframe]:w-full" dangerouslySetInnerHTML={{ __html: detailActivity.location_maps }} /></div></div>
            <div className="p-6 mt-6 bg-white rounded-lg shadow-md"><h2 className="mb-4 text-2xl font-bold text-gray-800">Ulasan Pengguna ({reviews.length})</h2><div className="space-y-6">{reviews.map((review) => (<div key={review.id} className="flex gap-4"><Avatar><AvatarImage src={review.user.avatarUrl} alt={review.user.name} /><AvatarFallback>{review.user.name.charAt(0)}</AvatarFallback></Avatar><div className="flex-1"><div className="flex items-center justify-between"><p className="font-semibold text-gray-800">{review.user.name}</p><div className="flex items-center gap-1 text-sm text-yellow-500"><Star className="w-4 h-4 fill-current" /><span>{review.rating.toFixed(1)}</span></div></div><p className="text-sm text-gray-500">{new Date(review.createdAt).toLocaleDateString("id-ID", { year: "numeric", month: "long", day: "numeric" })}</p><p className="mt-2 text-gray-700">{review.comment}</p></div></div>))}{user && <ReviewForm activityId={initialActivity.id} />}</div></div>
          </div>
          <div className="lg:w-1/3">
            <div className="sticky top-24">
              <div className="p-6 bg-white rounded-lg shadow-md">
                <div className="mb-4">{displayPrice > 0 ? (<>{showStrikethrough && (<span className="text-lg text-red-500 line-through">{new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", minimumFractionDigits: 0 }).format(price)}</span>)}<p className={`text-3xl font-bold ${showStrikethrough ? "text-blue-600" : "text-gray-900"}`}>{new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", minimumFractionDigits: 0 }).format(displayPrice)}</p><p className="text-sm text-gray-600">/ orang</p></>) : (<p className="text-xl font-bold text-gray-700">Harga tidak tersedia</p>)}</div>
                {displayPrice > 0 && (
                  <>
                    <div className="space-y-4">
                      <div className="grid w-full gap-1.5"><Label htmlFor="date">Pilih Tanggal</Label><Popover><PopoverTrigger asChild><Button id="date" variant={"outline"} className={cn("w-full justify-start text-left font-normal", !selectedDate && "text-muted-foreground")}><CalendarIcon className="w-4 h-4 mr-2" />{selectedDate ? format(selectedDate, "PPP", { locale: localeEn }) : <span>Pilih tanggal</span>}</Button></PopoverTrigger><PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={selectedDate} onSelect={setSelectedDate} disabled={(date) => date < new Date(new Date().setHours(0, 0, 0, 0))} initialFocus /></PopoverContent></Popover></div>
                      <div className="grid w-full gap-1.5"><Label htmlFor="quantity">Jumlah Orang / Kuantitas</Label><div className="flex items-center justify-between p-2 border rounded-md"><Button variant="outline" size="icon" onClick={handleDecrement} disabled={quantity <= 1}><Minus className="w-4 h-4" /></Button><span className="w-16 font-bold text-center text-md">{quantity}</span><Button variant="outline" size="icon" onClick={handleIncrement}><Plus className="w-4 h-4" /></Button></div></div>
                    </div>
                    ...
                    <Button size="lg" className="w-full text-lg bg-blue-600 hover:bg-blue-700" disabled={!selectedDate || isAddingToCart} onClick={handleAddToCart}>{isAddingToCart ? (<><Loader2 className="w-6 h-6 mr-2 animate-spin" /> Menambahkan...</>) : ("Tambah ke Keranjang")}</Button>
                    <p className="mt-2 text-xs text-center text-gray-500">Pembatalan Mudah & Pengembalian Dana Penuh</p>
                  </>
                )}
              </div>
            </div>
        ...
        {activityByCategory && activityByCategory.length > 0 && (
          <div className="mt-12">
            <h2 className="mb-6 text-2xl font-bold text-gray-800">Aktivitas Serupa</h2>
                                ...
                                <Link href={`/activities/${rec.id}`} className="block h-full group">
                                    <div className="flex flex-col h-full overflow-hidden transition-shadow bg-white rounded-lg shadow-md hover:shadow-xl">
                                        ...
                                        <div className="flex flex-col flex-grow p-4">
                                            ...
                                            <p className="text-sm text-yellow-500"><Star className="inline w-3 h-3 mr-1 fill-current" /> {rec.rating || "-"} ({rec.total_reviews || "0"} Ulasan)</p>
                                            <p className="pt-2 mt-auto text-lg font-bold text-blue-500">{new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", minimumFractionDigits: 0 }).format(rec.price ?? 0) || "Harga tidak tersedia"}</p>
                                        </div>
                                    </div>
                                </Link>
                            </motion.div>

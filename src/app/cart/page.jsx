...
const CartPage = () => {
  ...
  const handleProcessPayment = async () => {
    ...
    try {
      ...
      if (err.response) {
        ...
        toast.error(`Error ${err.response.status}: ${err.response.data?.message || 'Failed to process order.'}`);
      } else {
  ...
  if (error) return <div className="flex flex-col items-center justify-center min-h-[80vh]"><Frown className="w-16 h-16 text-red-500" /><h2 className="mt-4 text-xl">Failed to load cart.</h2><p>{error}</p></div>;
  if (cartItems.length === 0) return <div className="flex flex-col items-center justify-center min-h-[80vh]"><ShoppingCart className="w-24 h-24 text-gray-300" /><h1 className="mt-6 text-3xl font-bold">Your Cart is Empty</h1><Button className="mt-4" onClick={() => router.push("/")}>Find Activities</Button></div>;
  ...
  return (
    <div className="container px-4 py-8 mx-auto lg:py-12">
      ...
      <Dialog open={isSingleItemDeleteDialogOpen} onOpenChange={setIsSingleItemDeleteDialogOpen}>
        <DialogContent className="sm:max-w-[425px]"><DialogHeader><DialogTitle>Delete Item?</DialogTitle><DialogDescription>Are you sure you want to delete "{currentItemForDeletion?.title}"?</DialogDescription></DialogHeader><DialogFooter><DialogClose asChild><Button variant="outline">Cancel</Button></DialogClose><Button variant="destructive" onClick={executeSingleItemDelete}>Yes, Delete</Button></DialogFooter></DialogContent>
      </Dialog>
      <h1 className="mb-8 text-3xl font-bold">Shopping Cart</h1>
      <div className="flex flex-col gap-8 lg:flex-row lg:items-start">
        <div className="w-full lg:w-2/3">
          <div className="flex items-center justify-between p-4 mb-4 bg-white border rounded-lg">
            <div className="flex items-center"><Checkbox id="select-all" checked={selectedItems.size === cartItems.length && cartItems.length > 0} onCheckedChange={handleSelectAll} /><label htmlFor="select-all" className="ml-3 text-sm font-medium">Select All ({cartItems.length} items)</label></div>
            <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}><DialogTrigger asChild><Button variant="ghost" className="text-sm text-red-600 hover:bg-red-50" disabled={selectedItems.size === 0}><Trash2 className="w-4 h-4 mr-2" />Delete</Button></DialogTrigger><DialogContent className="sm:max-w-[425px]"><DialogHeader><DialogTitle>Confirm</DialogTitle><DialogDescription>Delete {selectedItems.size} selected items?</DialogDescription></DialogHeader><DialogFooter><DialogClose asChild><Button variant="outline">Cancel</Button></DialogClose><Button variant="destructive" onClick={executeDeleteSelected}>Yes, Delete</Button></DialogFooter></DialogContent></Dialog>
          </div>
        ...
        <div className="w-full lg:w-1/3">
          <div className="sticky p-6 bg-white border rounded-lg shadow-sm top-24">
            ...
            <div className="space-y-2">
              ...
              {appliedPromo && (<div className="flex justify-between text-green-600"><span>Promo Discount</span><span className="font-medium">- Rp {discount.toLocaleString("id-ID")}</span></div>)}
            ...
            <p className="mt-2 text-sm text-gray-500">For {selectedItems.size} selected items</p>
            ...
            <Button onClick={handleProcessPayment} size="lg" className="w-full mt-6 text-lg bg-blue-600 hover:bg-blue-700" disabled={selectedItems.size === 0 || !selectedPaymentMethod || isProcessing}>
              {isProcessing ? <><Loader2 className="w-6 h-6 mr-2 animate-spin" /> Processing...</> : "Proceed to Payment"}
            </Button>
